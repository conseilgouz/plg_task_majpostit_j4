...
class Majpostit extends CMSPlugin implements SubscriberInterface
{
    ...
    //
    public function upd_Post_it($st_type, $date_end, $st_desc)
    {
        ...
        foreach ($result as $res) {
            $st =  mb_convert_encoding($st_desc, 'UTF-8', 'ISO-8859-1');
    ...
    public function getJEvent($type)
    {
        ...
        if ((strlen($res['description']) == 0) || ($this->myparams->typeaff == "title")) {
            ...
            if (!(date('H\hi', strtotime($res['dtstart'])) == '00h00')) {
                $desc .= htmlentities(' à partir de '.date('H\hi', strtotime($res['dtstart'])), ENT_NOQUOTES, 'utf-8');
            }
        } else {
    ...
    public function getDPCalendar($type)
    {
        ...
        if ((strlen($res['description']) == 0) || ($this->myparams->typeaff == "title")) {
            ...
            if ($res['all_day'] = '0') {
                $desc .= htmlentities(' à partir de '.date('H\hi', strtotime($res['start_date'])), ENT_NOQUOTES, 'utf-8');
            }
        } else {
            // on limite la taille pour le post-it: 1ere paragraphe
            $desc = substr($res['description'], 0, strpos($res['description'], '</p>'));
    ...
    public function getDPCalendars()
    {
